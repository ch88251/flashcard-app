...
export default async function handler(req, res) {
  try {
    ...
    if (parts.length === 2) {
      ...
      if (method === 'POST') {
        // Mutations allowed on production; require auth below
        ...
        if (!['sentence', 'list', 'code'].includes(back_format)) {
          return res.status(400).json({ error: 'back_format must be one of "sentence", "list", or "code"' });
        }
        try {
          const created = await statements.insertFlashcard(category_id, front, back, back_format, body?.code_language);
    ...
    if (parts.length === 3) {
      ...
      if (method === 'PUT') {
        ...
        const updated = await statements.updateFlashcard(id, front, back, back_format, body?.code_language);
